...
const parseDischarge = (discharge: any): Discharge => {
  if (!discharge || (Object.keys(discharge).length === 0 && discharge.constructor === Object)) {
    return discharge
  } else {
    if (!discharge.date) {
...
const parseSickLeave = (sickleave: any): SickLeave => {
  if (!sickleave || (Object.keys(sickleave).length === 0 && sickleave.constructor === Object)) {
    return sickleave
  } else {
...
export const toNewEntry = (newEntry: any): NewEntry => {
  ...
  switch (validEntryType.type) {
    ...
    case 'HealthCheck':
      return {
        ...entry,
        healthCheckRating: parseHealthCheckRating(validEntryType.healthCheckRating),
        type: validEntryType.type,
      }
